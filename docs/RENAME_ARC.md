...
- **Admin Cities needs a status visibility/toggle UI.** Right now `/admin/cities` doesn't display the status field, has no Feature / Unfeature button, and the edit modal has no status field. After Job 9 the status field is what gates user-facing visibility, but admin can't see or change it from the UI. Workaround until shipped: promote/demote cities via SQL: `UPDATE cities SET status = 3 WHERE id = N` (Job 9 also added `status` to the PATCH allowed list, so a curl/devtools PATCH works too). Real fix is small — add a status column to the row display, add a status select (or two buttons "Feature" / "Unfeature") to the edit modal. Could ship standalone before Job 8 if it becomes urgent; otherwise bundle with Job 8 admin triage UI.
...
- **Two-field capture API path.** ✅ Shipped 2026-05-18 alongside the v0.6 capture overlay redesign. POST /api/spots accepts `{place_name, tip}` body fields and routes to `createSpotStructured` which skips the v1 single-phrase AI parse in favor of a tighter `parseCaptureStructured` call (category + neighborhood only). The `{ text }` single-phrase path stays for Shortcut/extension/voice callers.
