(function () {
  ...
  if (window.AdminShell) AdminShell.render('note-categories');
  ...
  async function load() {
    ...
    try {
      const data = await api.get('/api/notes/categories?includeInactive=true');
  ...
  function persistOrder() {
    ...
    api.patch('/api/notes/categories/reorder', { order: ids }).catch(() => {});
  }
  ...
  if (reassignBtn) {
    reassignBtn.addEventListener('click', async () => {
      ...
      try {
        const r = await api.post('/api/notes/categories/reassign', { from, to });
